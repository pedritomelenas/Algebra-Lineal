...
Sean $B=\{\mathbf{v}_1,\ldots, \mathbf{v}_n\}$ una base de $V$. Decimos que $B$ es una *base ortogonal* de $V$ si sus vectores son ortogonales dos a dos; es decir, si $\langle \mathbf{v}_i, \mathbf{v}_j \rangle=0$ para todo $i\neq j$.

Decimos que $B$ es una *base ortonormal* de $V$ si es una base ortogonal y, además, todos sus vectores son unitarios; es decir, si $\langle \mathbf{v}_i, \mathbf{v}_j \rangle=0$ para todo $i\neq j$, y $\Vert \mathbf{v}_i\Vert=1$ para todo $i\in\{1,\ldots,n\}$.

Si $B=\{\mathbf{v}_1,\ldots, \mathbf{v}_n\}$ es una base ortonormal de $V$, entonces para todo $\mathbf{u}\in V$ se verifica que
$$
...
Sean $(V;\langle \cdot, \cdot \rangle)$ un espacio vectorial euclídeo y $B=\{\mathbf{v}_1,\ldots, \mathbf{v}_n\}$ una base dada de $V$.

Los vectores $\{\mathbf{u}_1,\mathbf{u}_2,\ldots,\mathbf{u}_n\}$ definidos por
$$
\begin{aligned}
...
$$
\left\{\frac{1}{\Vert \mathbf{u}_1\Vert}\mathbf{u}_1,\ldots,\frac{1}{\Vert \mathbf{u}_n\Vert}\mathbf{u}_n\right\}
$$
...
Sea $S=\{\mathbf{u}_1,\ldots,\mathbf{u}_m\}$ una base de $U$. Entonces, por linealidad,
tenemos que
$$
\mathbf{v}\perp U \text{ si, y sólo si, } \langle \mathbf{v}, \mathbf{u}_i \rangle=0 \text{ para todo } i\in\{1,\ldots,m\}.
$$
...
Consideremos el subespacio vectorial $U \subseteq \mathbb{R}^4$ generado por los vectores $\left\{ (6,3,2,0),(4,1,0,-2) \right\}$.Entonces las ecuaciones cartesianas de $U^\perp$ son
$$
\left\{
\begin{array}{r} 
6x_1+3x_2+2x_3=0, \\ 
...
\end{array}\right.
$$
Además, $\left\{ (1,-4,3,0),(1,-2,0,1) \right\}$ es una base de $U^\perp$.

## Proyección ortogonal

En un espacio vectorial euclídeo $(V;\langle \cdot, \cdot \rangle)$, consideremos un conjunto de vectores linealmente independientes $S=\{\mathbf{u}_1,\ldots,\mathbf{u}_m\}\subset V$. Además, sean $U=\mathcal{L}(S)$ y $\{\mathbf{u}_{m+1},\ldots,\mathbf{u}_n\}$ una base de $U^\perp$.
...
- Consideramos $\{\mathbf{u}_1,\ldots,\mathbf{u}_n\}$ base ortogonal de $(V;\langle \cdot, \cdot \rangle)$ tal que $\{\mathbf{u}_1,\ldots,\mathbf{u}_m\}$ es base de $U$ y $\{\mathbf{u}_{m+1},\ldots,\mathbf{u}_n\}$ es base de $U^\perp$.
...
- Supongamos ahora que $(V;\langle \cdot, \cdot \rangle)$ es un espacio vectorial euclídeo y que $S=\{\mathbf{u}_1,\ldots,\mathbf{u}_m\}$ es una base (no necesariamente ortogonal) de un subespacio vectorial $U$ de $V$.
    ...
    $$
    \langle \mathbf{v}-a_1\mathbf{u}_1-\cdots-a_m \mathbf{u}_m,\mathbf{u}_j \rangle=0, j\in\{1,\ldots,m\},
    $$
...
$$
G = \begin{pmatrix} \langle \mathbf{u}_1,\mathbf{u}_1 \rangle & \cdots & \langle \mathbf{u}_m,\mathbf{u}_1 \rangle \\ \vdots & & \vdots \\ \langle \mathbf{u}_1,\mathbf{u}_m \rangle & \cdots & \langle \mathbf{u}_m,\mathbf{u}_m \rangle 
\end{pmatrix},
$$
se le conoce como *matriz de Gram* de $\langle \cdot,\cdot\rangle$ asociada a la base $\{\mathbf{u}_1,\ldots,\mathbf{u}_m\}$ de $U$.

Si $B=\{\mathbf{v}_1,\dots,\mathbf{v}_n\}$ es una base de $V$, y $G$ es la matriz de Gram de $\langle \cdot,\cdot\rangle$ respecto de esa base, para cualesquiera $\mathbf{u}$ y $\mathbf{v}$ de $V$ con coordenadas $(u_1,\dots,u_n)$ y $(v_1,\dots,v_n)$, se tiene que 
$$
